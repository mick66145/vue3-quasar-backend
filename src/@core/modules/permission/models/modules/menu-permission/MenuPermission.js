...
const MenuPermissionFactory = (item = null) => {
    const factory = (item) => {
        return {
            id: item?.id || "",
            name: item?.name || "",
            childs: item?.childs ? [...item?.childs].map(element => {
                const menuPermissionObj = MenuPermissionFactory(element)
    ...
    return factory(item || MenuPermission);
}

export default MenuPermissionFactory
